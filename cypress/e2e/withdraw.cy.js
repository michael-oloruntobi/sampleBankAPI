describe("Withdraw API Test", () => {
  ...
  it("send a withdraw request successfully", function () {
    cy.request({
      method: "POST",
      url: this.endpoints.withdraw,
      ...
      expect(response.body.message).to.equal("Withdrawal request processed.");
      ...
      expect(response.body.data.amount).to.equal(50);
      expect(response.body.data.transaction_id).to.equal("456XYZ789ABC");
    });
  });
